...
impl SubtreeLeafScores {
    fn new(size: usize, class_number: usize) -> Self {
        Self {
...
impl  ConTreeDepth2 {
    ...
    fn expand_feature_subtree(&self, view: &DataView, feature: usize, config: &SearchConfig, entry: &mut Entry, upper_bound: usize, tree: &mut Tree, stats: &mut Statistics) {
        ...
        if possible_split_indices.len() == 0 {
            return;
        }

        let mut pruner = IntervalsPruner::new(possible_split_indices, config.max_gap);

        let mut queue = VecDeque::new();
        let init_bound = Bound::new(0, possible_split_indices.len() - 1, None, None);
        queue.push_back(init_bound);

        while !queue.is_empty() {
            let mut current_bound = queue.pop_front().unwrap();
            if pruner.subinterval_pruning(&current_bound, entry.error) {
                continue;
            }

            pruner.interval_shrinking(&mut current_bound, entry.error);
    ...
    fn process_single_point(
        ...
        // Update if improved
        if total_score > 0 && total_score as usize > tree.classification_score {
